...
<head>
        ...
        <!-- ======== Page title ============ -->
        <title>Products - Vetri Velan Kaappi</title>
        <!--<< Favcion >>-->
                    ...
                    <div class="mega-menu-wrapper">
                        <div class="header-main">
                            <div class="logo">
                                ...
                                <img src="assets/img/logo/logo_tamil.png" alt="logo-img" width="240">
                                </a>
                            </div>
                                ...
                                <div class="main-menu">
                                    <nav id="mobile-menu" style="display: block;">
                                        <ul>
                                            ...
                                            <li>
                                                <a href="products.php" class="active-new">Products</a>
                                            </li>
                                            <li>
...
<section class="food-category-section fix section-padding section-bg">
    <div class="container">
        ...
        <?php
        // ---------- Helper: decide ORDER BY column ----------
        $orderColumn = 'product_id';
        // check if created_at column exists and use it when available
        $colCheck = mysqli_query($conn, "SHOW COLUMNS FROM products LIKE 'created_at'");
        if (mysqli_num_rows($colCheck) > 0) {
            $orderColumn = 'created_at';
        }

        // ---------- Fetch all active categories ----------
        $catSQL = mysqli_query($conn, "SELECT * FROM categories WHERE status = 1 ORDER BY category_id ASC");
        while ($cat = mysqli_fetch_assoc($catSQL)) {

            $cat_id   = (int)$cat['category_id'];
            $cat_name = htmlspecialchars($cat['category_name']);
            $cat_image = htmlspecialchars($cat['category_image']);
            // ---------- Get latest 3 product IDs for THIS category ----------
            $newProducts = [];
            if ($orderColumn === 'created_at') {
                // use created_at (newest by timestamp)
                $newQ = mysqli_query($conn, "
                    SELECT product_id 
                    FROM products 
            ...
            while ($r = mysqli_fetch_assoc($newQ)) {
                $newProducts[] = (int)$r['product_id'];
            }

            // ---------- Optional debug (uncomment to see latest ids) ----------
            // echo "<pre>Category: $cat_name - New IDs: "; print_r($newProducts); echo "</pre>";

            // ---------- Fetch all active products for this category ----------
            if ($orderColumn === 'created_at') {
            ...
            if (!mysqli_num_rows($prodSQL)) {
                continue;
            }
        ?>

            <!-- CATEGORY TITLE -->
            <div class="section-title mt-5 mb-3" style="display:flex; gap:10px">
                <img src="admin/uploads/category/<?= $cat_image ?>" alt="<?= $cat_name ?>" style="width:50px; border-radius:50px"><h3 style="font-weight:700;"><?= $cat_name ?></h3>
            </div>
                            <p style="font-size:18px; color:#D50004;">
                    <?= !empty($cat['description']) ? htmlspecialchars($cat['description']) : "Explore our freshly prepared {$cat_name} items."; ?>
                </p>

            <div class="row mb-5">

                <?php while ($prod = mysqli_fetch_assoc($prodSQL)) {
                    $pid = (int)$prod['product_id'];
                    $pname = htmlspecialchars($prod['product_name']);
                    $pdesc = htmlspecialchars($prod['description']);
                    $pimg  = !empty($prod['product_image']) ? "admin/uploads/products/".htmlspecialchars($prod['product_image']) : "assets/img/default-product.png";
                ?>

                    <div class="col-xl-3 col-lg-4 col-md-6 mb-4">
                        <div class="catagory-product-card-2 shadow-style text-center" style="position:relative">

                            <!-- NEW TAG for latest 3 PRODUCTS IN THIS CATEGORY -->
                            <?php if (in_array($pid, $newProducts, true)) { ?>
                    <div class="icon" style="color:white;">
                        New
                            ...
                            <div class="catagory-product-image">
                                <center>
                                    <img src="<?= $pimg ?>" alt="<?= $pname ?>" style="width:230px; border-radius:10px;">
                                </center>
                            </div>
                            ...
                            <div class="catagory-product-content">
                                ...
                                <p style="
                                    ...
                                    border-radius:6px;
                                ">
                                    <?= nl2br(substr($pdesc, 0, 550)) ?><?= (strlen($pdesc) > 550 ? '...' : '') ?>
                                </p>
                            </div>
